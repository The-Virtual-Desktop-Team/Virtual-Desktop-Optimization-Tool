...
Function Optimize-DefaultUserSettings
{
    ...
    Process
    {
        ...
        {
            Write-EventLog -EventId 1 -Message "Set Default User Settings" -LogName 'Virtual Desktop Optimization' -Source 'VDOT' -EntryType Information
            Write-Verbose "[VDI Optimize] Set Default User Settings"
            ...
            {
                Write-EventLog -EventId 1 -Message "Processing Default User Settings (Registry Keys)" -LogName 'Virtual Desktop Optimization' -Source 'VDOT' -EntryType Information
                ...
                Foreach ($Item in $UserSettings)
                {
                    ...
                    {
                        Write-EventLog -EventId 1 -Message "Found $($Item.HivePath) - $($Item.KeyName)" -LogName 'Virtual Desktop Optimization' -Source 'VDOT' -EntryType Information        
                        Write-Verbose "Found $($Item.HivePath) - $($Item.KeyName)"
                        If (Get-ItemProperty -Path ("{0}" -f $Item.HivePath) -ErrorAction SilentlyContinue)
                        {
                            Write-EventLog -EventId 1 -Message "Set $($Item.HivePath) - $Value" -LogName 'Virtual Desktop Optimization' -Source 'VDOT' -EntryType Information
                            Set-ItemProperty -Path ("{0}" -f $Item.HivePath) -Name $Item.KeyName -Value $Value -Force 
                        }
                        Else
                        {
                            Write-EventLog -EventId 1 -Message "New $($Item.HivePath) Name $($Item.KeyName) PropertyType $($Item.PropertyType) Value $Value" -LogName 'Virtual Desktop Optimization' -Source 'VDOT' -EntryType Information
                            New-ItemProperty -Path ("{0}" -f $Item.HivePath) -Name $Item.KeyName -PropertyType $Item.PropertyType -Value $Value -Force | Out-Null
                        }
                    }
                    Else
                    {
                        Write-EventLog -EventId 1 -Message "Registry Path not found $($Item.HivePath)" -LogName 'Virtual Desktop Optimization' -Source 'VDOT' -EntryType Information
                        Write-EventLog -EventId 1 -Message "Creating new Registry Key $($Item.HivePath)" -LogName 'Virtual Desktop Optimization' -Source 'VDOT' -EntryType Information
                        ...
                        Else
                        {
                            Write-EventLog -EventId 1 -Message "Failed to create new Registry Key" -LogName 'Virtual Desktop Optimization' -Source 'VDOT' -EntryType Error
                        } 
                    }
                }
            ...
            Else
            {
                Write-EventLog -EventId 1 -Message "No Default User Settings to set" -LogName 'Virtual Desktop Optimization' -Source 'VDOT' -EntryType Warning
            }
        }
        Else
        {
            Write-EventLog -EventId 1 -Message "File not found: $DefaultUserSettingsFilePath" -LogName 'Virtual Desktop Optimization' -Source 'VDOT' -EntryType Warning
        }
    }
